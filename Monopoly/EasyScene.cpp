...
void EasyScene::paint()
{
	...
	/***************************************规则与提示的显示***************************************/
	hdc = GetDC(hWnd);
	SetTextColor(hdc, RGB(255, 0, 0));          //设置文本颜色、背景色、大小
...
void EasyScene::allStartMove()
{
	...
	PLAYER_TYPE deleteArr[4];               //最多就4个玩家,直接硬编码成4了，懒
	int cc = 0;
	//检查有没有破产的玩家，直接剔除他
	//原本我是在每移动一个玩家就进行一次单独的判断，但发现由于结构限制那样做会造成重大bug，故放弃
	for (int i = 0; i < playerManager->realPlayerCount; ++i)
